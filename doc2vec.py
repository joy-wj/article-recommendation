...
def load_glove(filename):
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            word = word_list[0]
            vector = np.array(word_list[1:], dtype=float)
            gloves[word] = vector
            readline = f.readline()
    return gloves
...
def load_articles(articles_dirname, gloves):
    ...
    for file in allfiles:
        if os.path.relpath(file, articles_dirname) != 'COPYRIGHT':
            ...
            title = line_list[0]
            cond = '\n'.join(line_list[1:])
            centroid = doc2vec(cond, gloves)
            record = [filename, title, cond, centroid]
...
def doc2vec(text, gloves):
    ...
    vector_sum = np.full((1, 300), 0, dtype=float)
    count = 0
...
def distances(article, articles):
    ...
    """
    dist_list = [(np.linalg.norm(article[3] - a[3]), a)for a in articles if a != article]
    return dist_list
...
def recommended(article, articles, n):
    ...
    sort_dist = sorted(dist_list)
    n_articles = [tup[1] for tup in sort_dist[:n]]
    return n_articles
